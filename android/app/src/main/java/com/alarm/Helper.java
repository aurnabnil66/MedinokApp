    ...
    static void scheduleAlarm(Context context, String alarmUid, long triggerAtMillis, int notificationID) {
        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                // Check for exact alarm permission on Android 12 and above
            ...
            // Handle SecurityException (e.g., fallback to inexact alarms)
            Log.e(TAG, "SecurityException: Exact alarm permission not granted. Falling back to inexact alarm.");
            alarmManager.set(AlarmManager.RTC_WAKEUP, triggerAtMillis, pendingIntent);
        }
