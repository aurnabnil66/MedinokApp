...
import {
  IWeeklyDoseTime,
  IXDaysDoseTime,
} from '../../store/slices/features/medicineDetails/types';
import {
  addscheduleList,
  setDoseList,
  setXDaysTakeDose,
...
const EveryXdaysDoseDetails: FC = () => {
  ...
  const xDaysDoseTime = useSelector(
    (state: RootState) => state.medicineDetails.xDaysDoseTime,
  );
  ...
  const handleNext: any = async () => {
    ...
    if (filterArray.length > 0) {
      ...
      let scheduleList = await localSchedule(
        dataArrary,
        'day',
        medicineLocalId,
      );
